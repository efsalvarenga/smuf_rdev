...
for (h in hrz_lim){
  ...
  wl06ag     <- fx_int_fcstgeneric_armagarch(wm01_01,h,in_sample_fr,s01,s02,sum_of_h,win_selec,is_wins_weeks,crossvalsize,T,armalags,cross_overh,gof.min)
  reslis.h[[1]] <- rbind(reslis.h[[1]],colMeans(wl06kdWS1[[2]],na.rm=T))
  reslis.h[[2]] <- rbind(reslis.h[[2]],colMeans(wl06kdWS2[[2]],na.rm=T))
  reslis.h[[3]] <- rbind(reslis.h[[3]],colMeans(wl06ag[[2]],na.rm=T))
  resmat.h  <- matrix(nrow=0,ncol=max(ahead_t))
  for (k in 1:length(reslis.h)){
    resmat.h <- rbind(resmat.h,colMeans(reslis.h[[k]],na.rm=T))
  }
  rownames(resmat.h) <- c(paste("KD",1:2),"AG")
