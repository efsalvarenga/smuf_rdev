...
savfile = "smuf_run_0720_defheur_clulight_int01.rds"
...
cus_list      <- seq(1,50)
frontierstp   <- 7             # Number of demand bins (Stepwise frontier for portfolio optimisation)
...
hrz_lim       <- 0#seq(1,10)*113*2            # Rolling forecasts steps {seq(0:167)*113} is comprehensive
...
bighlpopgr <- list()
bighlpcrps <- list()
myleg = c("Random","sdev","seas non-scaled focus","seas scaled focus","seas non-scaled all","seas scaled all")
...
for (h in hrz_lim){
  ...
  optgrp_sfnf  <- foreach (i = 1:frontierstp,
                           .packages=c("forecast","rgenoud"),
                           .combine=c("rbind")) %dopar% {
                             opt_min_cusd  = wv46[i]
                             opt_max_cusd  = wv46[i+1]
  ...
  bighlpopgr   <- fx_sav_optgrps(c("sfnf",h,frontierstp,length(cus_list),crossvalstps,armalags,runkey),optgrp_sfnf)
  res_sfnf_ag  <- fx_applgrp(optgrp_sfnf,wv46,wm01_01,fx_int_fcstgeneric_armagarch,h,in_sample_fr,s01,s02,sum_of_h,win_size,is_wins_weeks,crossvalsize,armalags,cross_overh,crossvalfocus)
  
  cat("[OptSEAF_sf] ")
  wm13seaf     <- as.matrix(wm13n[,crossvalfocus])
  optgrp_sfsf  <- foreach (i = 1:frontierstp,
                           .packages=c("forecast","rgenoud"),
                           .combine=c("rbind")) %dopar% {
                             opt_min_cusd  = wv46[i]
                             opt_max_cusd  = wv46[i+1]
  ...
  bighlpopgr   <- fx_sav_optgrps(c("sfsf",h,frontierstp,length(cus_list),crossvalstps,armalags,runkey),optgrp_sfsf)
  res_sfsf_ag  <- fx_applgrp(optgrp_sfsf,wv46,wm01_01,fx_int_fcstgeneric_armagarch,h,in_sample_fr,s01,s02,sum_of_h,win_size,is_wins_weeks,crossvalsize,armalags,cross_overh,crossvalfocus)
  ...
  cat("[OptSEAF_sa] ")
  wm13seaf     <- as.matrix(wm13n)
  optgrp_sfsa  <- foreach (i = 1:frontierstp,
                           .packages=c("forecast","rgenoud"),
                           .combine=c("rbind")) %dopar% {
                             opt_min_cusd  = wv46[i]
                             opt_max_cusd  = wv46[i+1]
  ...
  bighlpopgr   <- fx_sav_optgrps(c("sfsa",h,frontierstp,length(cus_list),crossvalstps,armalags,runkey),optgrp_sfsa)
  res_sfsa_ag  <- fx_applgrp(optgrp_sfsa,wv46,wm01_01,fx_int_fcstgeneric_armagarch,h,in_sample_fr,s01,s02,sum_of_h,win_size,is_wins_weeks,crossvalsize,armalags,cross_overh,crossvalfocus)
  
  bighlpcrps   <- fx_sav_optress(c("sdev+seafs",h,frontierstp,length(cus_list),crossvalstps,armalags,crossvalfocus,runkey),
                                   list(c(h,frontierstp,length(cus_list)),cbind(cr01rnd,wv45rnd),res_sdev_ag,res_sfnf_ag,res_sfsf_ag,res_sfna_ag,res_sfsa_ag))
