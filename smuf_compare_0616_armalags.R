...
savfile = "smuf_compare_0616_armalags.rds"
...
for (h in hrz_lim){
  ...
  for (g in 1:length(armalagsseq)){
    reslis.h[[g]] <- rbind(reslis.h[[g]],colMeans(wl06ag[[g]][[2]],na.rm=T))
  }
  reslis.h[[(length(armalagsseq)+1)]] <-  rbind(reslis.h[[(length(armalagsseq)+1)]],colMeans(wl06kdWS1[[2]],na.rm=T))
  reslis.h[[(length(armalagsseq)+2)]] <-  rbind(reslis.h[[(length(armalagsseq)+2)]],colMeans(wl06kdWS2[[2]],na.rm=T))
  resmat.h  <- matrix(nrow=0,ncol=max(ahead_t))
