...
savfile = "smuf_run_0815_sd_gen-vs-qp.rds"
...
hrz_lim       <- seq(0,10)*113*7            # Rolling forecasts steps {seq(0:167)*113} is comprehensive
...
bighlpopgr <- list()
bighlpcrps <- list()
myleg = c("Random","sdev_bin","sdev_rlx")
...
for (h in hrz_lim){
  ...
  res_sdrlx_ag <- fx_applgrp(optgrp_sdrlx,wv46,wm01_01,fx_int_fcstgeneric_armagarch,h,in_sample_fr,s01,s02,sum_of_h,win_size,is_wins_weeks,crossvalsize,armalags,cross_overh,crossvalfocus)
  
  bighlpcrps   <- fx_sav_optress(c("sdev-vs-rlx",h,frontierstp,length(cus_list),crossvalstps,armalags,crossvalfocus,runkey),
