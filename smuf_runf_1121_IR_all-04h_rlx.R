...
savfile = "smuf_runf_1121_IR_all-04h_rlx.rds"
...
aht_selec     <- 4
hrz_lim       <- seq(1,10)*29            # Rolling forecasts steps {seq(0:167)*113} is comprehensive
...
for (h in hrz_lim){
  ...
  cat("[OptSDrlx] ")
  optgrp_sdrlx <- foreach (i = 1:frontierstp,
                           .packages=c("forecast","rgenoud"),
                           .combine=c("rbind")) %dopar% {
                             opt_min_cusd  = wv46[i]
                             opt_max_cusd  = wv46[i+1]
  ...
  optgrp_sdrlx <- optgrp_sdrlx/10
  bighlpopgr   <- fx_sav_optgrps(c("sdrlx",h,frontierstp,length(cus_list),crossvalstps,armalags,runkey),optgrp_sdrlx)
  res_sdrlx    <- fx_applgrp(optgrp_sdrlx,wv46,wm01_01,fx_int_fcstgeneric_kdss,h,in_sample_fr,s01,s02,sum_of_h,win_size,is_wins_weeks,crossvalsize,armalags,cross_overh,crossvalfocus)
  
  bighlpcrps   <- fx_sav_optress(c("sdev+rlx",h,frontierstp,length(cus_list),crossvalstps,armalags,crossvalfocus,runkey),
                                 list(c(h,frontierstp,length(cus_list)),cbind(cr01rnd,wv45rnd),res_sdev,res_sdrlx))
  
  saveRDS(list(bighlpopgr,bighlpcrps),  file=savfile)
  
  fx_plt_rnd_vs_opt(bighlpcrps[[length(bighlpcrps)]][[2]],c(0,0.5),c(0,sum(wv45)),myleg,"CRPS")
