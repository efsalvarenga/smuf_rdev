...
savfile = "smuf_run_0801_defheur_cv1d.rds"
...
for (h in hrz_lim){
  ...
  cat("[OptCV7dKD] ")
  crossvalsize  <- 1
  optgrp_cv7dkd <- foreach (i = 1:frontierstp,
  ...
  bighlpopgr   <- fx_sav_optgrps(c("cv7dkd",h,frontierstp,length(cus_list),crossvalstps,armalags,crossvalfocus,runkey),optgrp_cv7dkd)
  res_cv7kd_ag <- fx_applgrp(optgrp_cv7dkd,wv46,wm01_01,fx_int_fcstgeneric_armagarch,h,in_sample_fr,s01,s02,sum_of_h,win_size,is_wins_weeks,crossvalsize,armalags,cross_overh,crossvalfocus)
  
  cat("[OptCV1dKD] ")
  crossvalsize  <- 1/7
  optgrp_cv1dkd <- foreach (i = 1:frontierstp,
                            .packages=c("forecast","rgenoud","foreach"),
                            .combine=c("rbind")) %dopar% {
                              opt_min_cusd  = wv46[i]
                              opt_max_cusd  = wv46[i+1]
  ...
  bighlpopgr   <- fx_sav_optgrps(c("cv1dkd",h,frontierstp,length(cus_list),crossvalstps,armalags,crossvalfocus,runkey),optgrp_cv1dkd)
  res_cv1kd_ag <- fx_applgrp(optgrp_cv1dkd,wv46,wm01_01,fx_int_fcstgeneric_armagarch,h,in_sample_fr,s01,s02,sum_of_h,win_size,is_wins_weeks,crossvalsize,armalags,cross_overh,crossvalfocus)
  
  cat("[OptCV7dAG] ")
  crossvalsize  <- 1
  optgrp_cv7dag <- foreach (i = 1:frontierstp,
                            .packages=c("forecast","rgenoud","foreach"),
                            .combine=c("rbind")) %dopar% {
                              opt_min_cusd  = wv46[i]
                              opt_max_cusd  = wv46[i+1]
  ...
  res_cv7ag_ag <- fx_applgrp(optgrp_cv7dag,wv46,wm01_01,fx_int_fcstgeneric_armagarch,h,in_sample_fr,s01,s02,sum_of_h,win_size,is_wins_weeks,crossvalsize,armalags,cross_overh,crossvalfocus)
  
  cat("[OptCV1dAG] ")
  crossvalsize  <- 1/7
  optgrp_cv1dag <- foreach (i = 1:frontierstp,
                            .packages=c("forecast","rgenoud","foreach"),
                            .combine=c("rbind")) %dopar% {
                              opt_min_cusd  = wv46[i]
                              opt_max_cusd  = wv46[i+1]
  ...
  bighlpopgr   <- fx_sav_optgrps(c("cv1dag",h,frontierstp,length(cus_list),crossvalstps,armalags,crossvalfocus,runkey),optgrp_cv1dag)
  res_cv1ag_ag <- fx_applgrp(optgrp_cv1dag,wv46,wm01_01,fx_int_fcstgeneric_armagarch,h,in_sample_fr,s01,s02,sum_of_h,win_size,is_wins_weeks,crossvalsize,armalags,cross_overh,crossvalfocus)
  
  bighlpcrps   <- fx_sav_optress(c("sdev+cv_kd+ag_7d+1d",h,frontierstp,length(cus_list),crossvalstps,armalags,crossvalfocus,runkey),
                                   list(c(h,frontierstp,length(cus_list)),cbind(cr01rnd,wv45rnd),res_sdev_ag,res_cv7kd_ag,res_cv1kd_ag,res_cv7ag_ag,res_cv1ag_ag))
