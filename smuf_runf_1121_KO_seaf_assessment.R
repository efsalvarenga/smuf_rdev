...
savfile = "smuf_runf_1121_KO_seaf_assessment.rds"
...
aht_selec     <- 4
hrz_lim       <- seq(267,277)*37            # Rolling forecasts steps {seq(0:167)*113} is comprehensive
...
crossvalfocus <- aht_selec                  # What period is focused when running crossvalidation
...
bighlpopgr <- list()
bighlpcrps <- list()
myleg = c("Random","sfna00","sfna10","sfna02","sfna05","sfna08")
...
for (h in hrz_lim){
  ...
  wv45rnd    <- as.numeric(wl06rnd[[1]][,aht_selec] * rowSums(wm01_02l[[2]]))
  # sd01rnd    <- as.numeric(fx_sd_mymat(wl06rnd[[3]]))
  cr01rnd    <- wl06rnd[[2]][,aht_selec]
  ...
  optgrp_sfna00  <- foreach (i = 1:frontierstp,
  ...
  bighlpopgr   <- fx_sav_optgrps(c("sfna00",h,frontierstp,length(cus_list),crossvalstps,armalags,runkey),optgrp_sfna00)
  res_sfna00_ag  <- fx_applgrp(optgrp_sfna00,wv46,wm01_01,fx_int_fcstgeneric_armagarch,h,in_sample_fr,s01,s02,sum_of_h,win_size,is_wins_weeks,crossvalsize,armalags,cross_overh,crossvalfocus)
  
  cat("[OptSEAF_na10] ")
  wm13seaf     <- as.matrix(wl06[[4]])
  optgrp_sfna10  <- foreach (i = 1:frontierstp,
                             .packages=c("forecast","rgenoud"),
                             .combine=c("rbind")) %dopar% {
                               opt_min_cusd  = wv46[i]
                               opt_max_cusd  = wv46[i+1]
  ...
  bighlpopgr   <- fx_sav_optgrps(c("sfna10",h,frontierstp,length(cus_list),crossvalstps,armalags,runkey),optgrp_sfna10)
  res_sfna10_ag  <- fx_applgrp(optgrp_sfna10,wv46,wm01_01,fx_int_fcstgeneric_armagarch,h,in_sample_fr,s01,s02,sum_of_h,win_size,is_wins_weeks,crossvalsize,armalags,cross_overh,crossvalfocus)
  ...
  bighlpcrps   <- fx_sav_optress(c("sdev+seafs_mix",h,frontierstp,length(cus_list),crossvalstps,armalags,crossvalfocus,runkey),
                                 list(c(h,frontierstp,length(cus_list)),cbind(cr01rnd,wv45rnd),res_sfna00_ag,res_sfna10_ag,res_sfna02_ag,res_sfna05_ag,res_sfna08_ag))
