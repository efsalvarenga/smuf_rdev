...
savfile = "smuf_runf_0919_KO_seaf167_mult.rds"
...
hrz_lim       <- seq(1,167)*37            # Rolling forecasts steps {seq(0:167)*113} is comprehensive
...
for (h in hrz_lim){
  ...
  cat("[OptSEAF0.25] ")
  optgrp_sf25  <- foreach (i = 1:frontierstp,
                             .packages=c("forecast","rgenoud"),
                             .combine=c("rbind")) %dopar% {
                               opt_min_cusd  = wv46[i]
                               opt_max_cusd  = wv46[i+1]
  ...
  bighlpopgr   <- fx_sav_optgrps(c("sf25",h,frontierstp,length(cus_list),crossvalstps,armalags,crossvalfocus,runkey),optgrp_sf25)
  res_sf25     <- fx_applgrp(optgrp_sf25,wv46,wm01_01,fx_int_fcstgeneric_armagarch,h,in_sample_fr,s01,s02,sum_of_h,win_size,is_wins_weeks,crossvalsize,armalags,cross_overh,crossvalfocus)

  cat("[OptSEAF0.50] ")
  optgrp_sf50  <- foreach (i = 1:frontierstp,
                           .packages=c("forecast","rgenoud"),
                           .combine=c("rbind")) %dopar% {
                             opt_min_cusd  = wv46[i]
                             opt_max_cusd  = wv46[i+1]
  ...
  bighlpopgr   <- fx_sav_optgrps(c("sf50",h,frontierstp,length(cus_list),crossvalstps,armalags,crossvalfocus,runkey),optgrp_sf50)
  res_sf50     <- fx_applgrp(optgrp_sf50,wv46,wm01_01,fx_int_fcstgeneric_armagarch,h,in_sample_fr,s01,s02,sum_of_h,win_size,is_wins_weeks,crossvalsize,armalags,cross_overh,crossvalfocus)

  cat("[OptSEAF0.75] ")
  optgrp_sf75  <- foreach (i = 1:frontierstp,
                           .packages=c("forecast","rgenoud"),
                           .combine=c("rbind")) %dopar% {
                             opt_min_cusd  = wv46[i]
                             opt_max_cusd  = wv46[i+1]
  ...
  bighlpopgr   <- fx_sav_optgrps(c("sf75",h,frontierstp,length(cus_list),crossvalstps,armalags,crossvalfocus,runkey),optgrp_sf75)
  res_sf75     <- fx_applgrp(optgrp_sf75,wv46,wm01_01,fx_int_fcstgeneric_armagarch,h,in_sample_fr,s01,s02,sum_of_h,win_size,is_wins_weeks,crossvalsize,armalags,cross_overh,crossvalfocus)  
    
  bighlpcrps   <- fx_sav_optress(c("sdev+2seas",h,frontierstp,length(cus_list),crossvalstps,armalags,crossvalfocus,runkey),
                                 list(c(h,frontierstp,length(cus_list)),cbind(cr01rnd,wv45rnd),res_sdev,res_sfna,res_sf25,res_sf50,res_sf75))
