...
rndgrp_plt <- foreach (i = 1:10, .packages=c("forecast"), .combine=c("rbind")) %dopar%{
  for (j in 1:rndsim_size){
    ...
    while (sum(rndgrp_pll)==0){rndgrp_pll = (runif(length(cus_list))<=(i/10))+0}
...
points(optgrp_plt[2:frontierstp,2],optgrp_plt[2:frontierstp,1],col="green",pch=19)
points(rndgrp_pltres[2:frontierstp,2],rndgrp_pltres[2:frontierstp,1],col="red",pch=19)
